...
function get_git_builtinvars()
    ...
    local cmds = {
        ...
        git_commit_date = {
            "log", "-1", "--date=format:%Y%m%d%H%M%S", "--format=%ad"
        }
    }
    ...
    if git then
        for name, argv in pairs(cmds) do
            ...
            if not result then result = "none" end
            builtinvars[name] = result:trim()
        end
    else
        builtinvars[name] =
            "not find git, please install git and add it to PATH."
    end
    return builtinvars
end
...
function main(target)
    ...
    title = "csp build system " .. tool_builtinvars["git_tag"] .. "+" ..
                tool_builtinvars["git_branch"] .. "." ..
                tool_builtinvars["git_commit"] ..
                ", A cross-platform build system based on xmake"
